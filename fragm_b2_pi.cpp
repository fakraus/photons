    ...
    void *userdata)
{
    ...
    static real x_min, x_max, Jx, xx, Egamma, Epion, beta;
    ...
    x_min = std::max(1.0l/(1.0l + eps/Egamma), 
                        1.0l/(1.0l + std::pow(mass_pion*std::cosh(y)/Egamma, 2)/rho2(y, P->energy)));
    x_max = 4.0l/(4.0l + std::pow(mass_pion/Egamma, 2));
    xx = x_min + x[3]*(x_max - x_min);
    Jx = std::max(0.0l, x_max - x_min);
    p /= xx;
    Epion = p * cosh(y);
    beta = std::sqrt(1.0l - std::pow(mass_pion/Epion, 2));
    ...
    P->pionFF_ptr(z, q2_fragm, DPU, DPD, DPS, DPC, DPG);
    ...
    //this gives d2sigma_dydp for pions in f[0]
